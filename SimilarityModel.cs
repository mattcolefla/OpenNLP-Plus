...
namespace OpenNLP.Tools.Coreference.Similarity
{
	...
	public class SimilarityModel : ITestSimilarityModel, ITrainSimilarityModel
	{
		...
		public virtual void SetExtents(Context[] extents)
		{
			...
			foreach (var currentExtent in extents)
			{
			...
			var allExtentsIndex = 0;
            var headSets = ConstructHeadSets(entities);
            var nameSets = ConstructNameSets(entities);
			
			foreach (var key in entities.Keys)
            {
                var entityNameSet = nameSets[key];
                ...
                var exclusionSet = ConstructExclusionSet(key, entities, headSets, nameSets, singletons);

                //if (entityContexts.Count == 1)
                //{
                //}
				for (var firstEntityContextIndex = 0; firstEntityContextIndex < entityContexts.Count; firstEntityContextIndex++)
				{
					var firstEntityContext = entityContexts[firstEntityContextIndex];
					//if (isPronoun(ec1)) {
					//  continue;
					//}
                    for (var secondEntityContextIndex = firstEntityContextIndex + 1; secondEntityContextIndex < entityContexts.Count; secondEntityContextIndex++)
					{
						var secondEntityContext = entityContexts[secondEntityContextIndex];
						//if (isPronoun(ec2)) {
						//  continue;
						//}
						AddEvent(true, firstEntityContext, secondEntityContext);
						var startIndex = allExtentsIndex;
						do 
						{
							var compareEntityContext = allExtents[allExtentsIndex];
							allExtentsIndex = (allExtentsIndex + 1) % allExtents.Count;
							if (!exclusionSet.Contains(compareEntityContext))
							{
								if (DebugOn)
								{
									Console.Error.WriteLine(firstEntityContext.ToString() + " " + string.Join(",", entityNameSet.ToArray()) + " " + compareEntityContext.ToString() + " " + nameSets[compareEntityContext.Id]);
								}
		...
		private void AddEvent(bool same, Context firstNounPhrase, Context secondNounPhrase)
		{
			if (same)
			{
				var features = GetFeatures(firstNounPhrase, secondNounPhrase);
		...
		private Util.Set<string> ConstructHeadSet(IEnumerable<Context> mentions)
		{
            ...
            foreach (var currentContext in mentions)
			{
		...
		{
			foreach (var currentEntityContext in entityContexts)
			{
                foreach (var currentCandidateContext in candidateContexts)
				{
             ...
             Dictionary<int, Util.Set<string>> headSets, Dictionary<int, Util.Set<string>> nameSets, IEnumerable<Context> singletons)
		{
            ...
            var entityHeadSet = headSets[entityKey];
            var entityNameSet = nameSets[entityKey];
			var entityContexts = entities[entityKey];

			//entities
			foreach (var key in entities.Keys)
            {
				var candidateContexts = entities[key];
			...
			foreach (var currentSingleton in singletons)
			{
				singles.Clear();
		...
		private bool InSuperClass(Context entityContext, Context candidateEntityContext)
		{
			...
			{
				var commonSynsetCount = 0;
				foreach (var synset in entityContext.Synsets)
                {
		...
		*/
		...
		{
			var features = GetFeatures(firstMention, secondMention);
            if (DebugOn)
            {
		...
		public virtual void TrainModel()
		{
			if (DebugOn)
			{
				...
				foreach (var trainingEvent in _events)
                {
		...
		{
            return nounPhrase.HeadTokenTag == PartsOfSpeechStrings.CardinalNumber;
		}
		
		private IEnumerable<string> GetNameCommonFeatures(Context name, Context common)
		{
            var synsets = common.Synsets;
        ...
        private IEnumerable<string> GetCommonPronounFeatures(Context common, Context pronoun)
		{
            ...
            var synsets = common.Synsets;
			var pronounText = pronoun.HeadTokenText.ToLower();
            var genderText = Resolver.AbstractResolver.GetPronounGender(pronounText);
		...
		{
            var features = new List<string>();
			var synsets1 = common1.Synsets;
            var synsets2 = common2.Synsets;
		...
		{
            var features = new List<string>();
            var firstGender = Resolver.AbstractResolver.GetPronounGender(pronoun1.HeadTokenText);
            var secondGender = Resolver.AbstractResolver.GetPronounGender(pronoun2.HeadTokenText);
		...
		private List<string> GetFeatures(Context np1, Context np2)
		{
		    ...
		    //  semantic categories
			var w1 = np1.HeadTokenText.ToLower();
			var w2 = np2.HeadTokenText.ToLower();
