...
namespace OpenNLP.Tools.Chunker
{
	...
	public class EnglishTreebankChunker : MaximumEntropyChunker
	{
		...
		public List<SentenceChunk> GetChunks(string[] tokens, string[] tags)
		{
			...
			{
				if (chunks[currentChunk].StartsWith("B-", StringComparison.Ordinal) || chunks[currentChunk] == "O")
                {
		...
		public List<SentenceChunk> GetChunks(string data)
		{
			...
			{
				var tokenAndTag = tokenAndTags[currentTokenAndTag].Split('/');
				tokens[currentTokenAndTag] = tokenAndTag[0];
                tags[currentTokenAndTag] = tokenAndTag.Length > 1 ? tokenAndTag[1] : PartsOfSpeechStrings.SentenceFinalPunctuation;
			}
