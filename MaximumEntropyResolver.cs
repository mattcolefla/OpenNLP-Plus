...
namespace OpenNLP.Tools.Coreference.Resolver
{
	...
	{		
        ...
        public override DiscourseEntity Resolve(Mention.MentionContext expression, DiscourseModel discourseModel)
		{
			...
			var entityIndex = 0;
			var nonReferentialProbability = NonReferentialResolver.GetNonReferentialProbability(expression);
			if (DebugOn)
			{
				Console.Error.WriteLine(ToString() + ".resolve: " + expression.ToText() + " -> " + "null " + nonReferentialProbability);
			}
			for (; entityIndex < GetNumberEntitiesBack(discourseModel); entityIndex++)
			{
				...
				if (IsExcluded(expression, discourseEntity))
				{
					...
					if (DebugOn)
					{
						Console.Error.WriteLine("excluded " + ToString() + ".resolve: " + expression.ToText() + " -> " + discourseEntity + " " + _candidateProbabilities[entityIndex]);
					}
				}
				else
				{
                    var features = GetFeatures(expression, discourseEntity).ToArray();
                    try
					{
			...
			// find max
			var maximumCandidateIndex = 0;
			for (var currentCandidate = 1; currentCandidate <= entityIndex; currentCandidate++)
			{
		...
		*/
		...
		{
            var entityContext = discourseEntity.LastExtent;
        ...
        private string GetSemanticCompatibilityFeature(Mention.MentionContext entityContext, DiscourseEntity discourseEntity)
		{
			...
			{
				double best = 0;
                foreach (var checkEntityContext in discourseEntity.Mentions)
				{
					var sim = SimilarityModel.AreCompatible(entityContext, checkEntityContext);
					if (DebugOn)
					{
		...
		{
            var entityGender = discourseEntity.Gender;
		...
		{
            var entityNumber = discourseEntity.Number;
        ...
        private IEnumerable<string> GetCompatibilityFeatures(Mention.MentionContext mention, DiscourseEntity entity)
		{
			...
			var semanticCompatibilityFeature = GetSemanticCompatibilityFeature(mention, entity);
			compatibilityFeatures.Add(semanticCompatibilityFeature);
			var genderCompatibilityFeature = GetGenderCompatibilityFeature(mention, entity);
			compatibilityFeatures.Add(genderCompatibilityFeature);
			var numberCompatibilityFeature = GetNumberCompatibilityFeature(mention, entity);
        ...
        private Util.Set<string> ConstructModifierSet(Mention.IParse[] tokens, int headIndex)
		{
			...
			for (var tokenIndex = 0; tokenIndex < headIndex; tokenIndex++)
			{
		...
		{
			token = token.ToLower();
			if (token == "the" || token == "these" || token == "these" || tag == PartsOfSpeechStrings.PossessivePronoun)
			{
				return true;
			}
			return false;
		...
		{
            var index = entityMentionStrip.IndexOf(mentionStrip);
			if (index != - 1)
			{
				//check boundries
				...
				var end = index + mentionStrip.Length;
		...
		{
            var features = new List<string>();
            var currentEntityContext = entity.LastExtent;
			var entityDistance = mention.NounPhraseDocumentIndex - currentEntityContext.NounPhraseDocumentIndex;
			var sentenceDistance = mention.SentenceNumber - currentEntityContext.SentenceNumber;
		...
		{
			var foundCompatiblePronoun = false;
			var foundIncompatiblePronoun = false;
			if (PartsOfSpeech.IsPersOrPossPronoun(mention.HeadTokenTag))
			{
                var pronounMap = GetPronounFeatureMap(mention.HeadTokenText);
				foreach (var candidateMention in entity.Mentions)
                {
					if (PartsOfSpeech.IsPersOrPossPronoun(candidateMention.HeadTokenTag))
					{
						...
						{
                            var candidatePronounMap = GetPronounFeatureMap(candidateMention.HeadTokenText);
							var allKeysMatch = true;
							foreach (var key in pronounMap.Keys)
                           {
        ...
        private string MentionString(Mention.MentionContext entityContext)
		{
			...
			for (var tokenIndex = 1; tokenIndex < mentionTokens.Length; tokenIndex++)
			{
        ...
        private string ExcludedTheMentionString(Mention.MentionContext entityContext)
		{
			...
			var first = true;
			var mentionTokens = entityContext.Tokens;
			foreach (var tokenObj in mentionTokens)
			{
        ...
        private string ExcludedDeterminerMentionString(Mention.MentionContext entityContext)
		{
			var output = new StringBuilder();
			var first = true;
            var mentionTokenParses = entityContext.TokenParses;
			for (var tokenIndex = 0; tokenIndex < mentionTokenParses.Length; tokenIndex++)
			{
                var token = mentionTokenParses[tokenIndex];
				var tag = token.SyntacticType;
				if (tag != PartsOfSpeechStrings.Determiner)
				{
					if (!first)
					{
						output.Append(" ");
        ...
        public static List<string> GetWordFeatures(Mention.IParse token)
		{
			...
			var wordFeature = string.Empty;
			if (EndsWithPeriod.IsMatch(word))
			{
                wordFeature = @",endWithPeriod";
            }
			
			var tokenTag = token.SyntacticType;
