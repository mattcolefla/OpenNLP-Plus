...
namespace OpenNLP.Tools.Coreference.Resolver
{
	...
	public abstract class AbstractResolver : IResolver
	{
		...
		{
            return Math.Min(discourseModel.EntityCount, mNumberEntitiesBack);
		}
        ...
        protected internal virtual int GetHeadIndex(Mention.MentionContext mention)
		{
			...
			for (var currentToken = mentionTokens.Length - 1; currentToken >= 0; currentToken--)
			{
                var token = mentionTokens[currentToken];
				if (token.SyntacticType != PartsOfSpeechStrings.PossessiveEnding 
                    && token.SyntacticType != PartsOfSpeechStrings.Comma 
                    && token.SyntacticType != PartsOfSpeechStrings.SentenceFinalPunctuation)
				{
					return currentToken;
				}
			}
        ...
        public virtual DiscourseEntity Retain(Mention.MentionContext mention, DiscourseModel discourseModel)
		{
			...
			for (; entityIndex < discourseModel.EntityCount; entityIndex++)
			{
                ...
                var candidateExtentContext = currentDiscourseEntity.LastExtent;
        ...
        protected internal virtual string GetFeatureString(Mention.MentionContext mention)
		{
            ...
            for (var currentToken = 1; currentToken < mentionTokens.Length; currentToken++)
			{
