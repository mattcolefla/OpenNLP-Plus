...
namespace OpenNLP.Tools.Coreference.Resolver
{
	...
	public class PluralNounResolver:MaximumEntropyResolver
	{
		...
		{
			var firstTok = mention.FirstTokenText.ToLower();
			var firstTokTag = mention.FirstToken.SyntacticType;
			var rv = mention.HeadTokenTag == PartsOfSpeechStrings.NounPlural && !IsDefiniteArticle(firstTok, firstTokTag);
			return rv;
		}
		...
		protected internal override bool IsExcluded(MentionContext mention, DiscourseEntity entity)
		{
			...
			else
			{
                ...
                return (cec.HeadTokenTag != PartsOfSpeechStrings.NounPlural || base.IsExcluded(mention, entity));
			}
		}
	}
}