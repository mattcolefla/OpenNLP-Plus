...
namespace OpenNLP.Tools.Coreference.Similarity
{
	...
	public class GenderModel : ITestGenderModel, ITrainSimilarityModel
	{
        ...
        {
            var features = GetFeatures(nounPhrase);
        ...
        virtual public void SetExtents(Context[] extents)
        {
            ...
            for (var currentExtent = 0; currentExtent < extents.Length; currentExtent++)
            {
                var extent = extents[currentExtent];
                if (extent.Id != -1)
                {
            ...
            foreach (var key in entities.Keys)
           {
                ...
                var gender = GetGender(entityContexts);
                if (gender != null)
                {
            ...
            //non-coref entities
            foreach (var entityContext in singletons)
            {
                var gender = GetGender(entityContext);
                if (gender == GenderEnum.Male)
                {
            ...
            foreach (var entityContext in males)
            {
                AddEvent(GenderEnum.Male.ToString(), entityContext);
            }
            
            foreach (var entityContext in females)
            {
                AddEvent(GenderEnum.Female.ToString(), entityContext);
            }
            
            foreach (var entityContext in eunuches)
            {
        ...
        private Util.Set<string> ReadNames(string nameFile)
		{
			...
			for (var line = nameReader.ReadLine(); line != null; line = nameReader.ReadLine())
			{
				names.Add(line);
			}
			return names;
		...
		private List<string> GetFeatures(Context nounPhrase)
		{
			...
			for (var tokenIndex = 0; tokenIndex < nounPhrase.HeadTokenIndex; tokenIndex++)
			{
			...
			{
				var tokens = nounPhrase.Tokens;
				for (var tokenIndex = 0; tokenIndex < nounPhrase.HeadTokenIndex || tokenIndex == 0; tokenIndex++)
				{
					var name = tokens[tokenIndex].ToString().ToLower();
            ...
            foreach (var synset in nounPhrase.Synsets)
            {
		...
		private GenderEnum GetGender(Context mention)
		{
			...
			var mentionTokens = mention.Tokens;
		...
		{
			foreach (var entityContext in entity)
           {
				var gender = GetGender(entityContext);
		...
		{
			var model = new GenderModel(modelName, false);
			
			var words = line.Split(' ');
			var dist = model.GenderDistribution(Context.ParseContext(words[0]));
			var output = "m=" + dist[model.MaleIndex] + " f=" + dist[model.FemaleIndex] + " n=" + dist[model.NeuterIndex] + " " + string.Join(",", (model.GetFeatures(Context.ParseContext(words[0])).ToArray()));
