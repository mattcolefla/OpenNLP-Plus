...
namespace OpenNLP.Tools.Graphs
{
    ...
    public class DirectedMultiGraph<V, E> : IGraph<V, E>
    {
        ...
        public DirectedMultiGraph(MapFactory<V, Dictionary<V, List<E>>> outerMapFactory, MapFactory<V, List<E>> innerMapFactory)
        {
            ...
            outgoingEdges = outerMapFactory.NewMap();
            incomingEdges = outerMapFactory.NewMap();
        }
        ...
        public DirectedMultiGraph(DirectedMultiGraph<V, E> graph) : this(graph.outerMapFactory, graph.innerMapFactory)
        {
            foreach ( /*Map.Entry<V, Dictionary<V, List<E>>>*/var map in graph.outgoingEdges /*.entrySet()*/)
            {
                var edgesCopy = innerMapFactory.NewMap();
                foreach ( /*Map.Entry<V, List<E>>*/var entry in map.Value)
                {
                    edgesCopy.Add(entry.Key, new List<E>(entry.Value));
                }
                outgoingEdges.Add(map.Key, edgesCopy);
            }
            foreach ( /*Map.Entry<V, Dictionary<V, List<E>>>*/var map in graph.incomingEdges)
            {
                ...
                incomingEdges.Add(map.Key, edgesCopy);
            }
        }
        ...
        public bool RemoveEdge(V source, V dest, E data)
        {
            ...
            var foundOut = outgoingEdges.ContainsKey(source) && outgoingEdges[source].ContainsKey(dest) && outgoingEdges[source][dest].Remove(data);
            var foundIn = incomingEdges.ContainsKey(dest) && incomingEdges[dest].ContainsKey(source) && incomingEdges[dest][source].Remove(data);
            if (foundOut && !foundIn)
            {
        ...
        public Set<V> GetNeighbors(V v)
        {
            ...
            if (children == null && parents == null)
                return null;
            var neighbors = innerMapFactory.NewSet();
        ...
        public List<E> GetAllEdges()
        {
            ...
            foreach (var e in outgoingEdges.Values)
            {
                foreach (var ee in e.Values)
                {
                    edges.AddRange(ee);
                }
            }
        ...
        {
            var childrenMap = outgoingEdges[source];
            if (childrenMap == null)
            {
                return new ReadOnlyCollection<E>(new E[] {});
            }
            var edges = childrenMap[dest];
            if (edges == null)
            {
                return new ReadOnlyCollection<E>(new E[] {});
            }
        ...
        public List<E> ConvertPath(List<V> nodes, bool directionSensitive)
        {
            ...
            var previous = nodeIterator.Current;
            while (nodeIterator.MoveNext())
            {
                var next = nodeIterator.Current;
                var connection = default(E);
        ...
        public int GetInDegree(V vertex)
        {
            ...
            var result = 0;
            var incoming = incomingEdges[vertex];
            foreach (var edges in incoming.Values)
            {
                result += edges.Count;
            }
            return result;
        ...
        {
            var result = 0;
            var outgoing = outgoingEdges[vertex];
            if (outgoing == null)
            {
                return 0;
            }
        ...
        {
            var map = innerMapFactory.NewMap();
