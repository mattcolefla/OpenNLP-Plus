...
namespace OpenNLP.Tools.Coreference
{
	...
	public abstract class AbstractLinker : ILinker
	{
		...
		{
		    var firstDiscourseEntity = discourseEntities[0];
			for (var discourseEntityIndex = 1; discourseEntityIndex < discourseEntities.Length; discourseEntityIndex++)
			{
			    var secondDiscourseEntity = discourseEntities[discourseEntityIndex]; //temporary variable
		...
		{
			var extentContexts = ConstructMentionContexts(mentions);
			var discourseModel = new DiscourseModel();
			for (var extentIndex = 0; extentIndex < extentContexts.Length; extentIndex++)
			{
		...
		{
			for (var resolverIndex = 0; resolverIndex < mResolvers.Length; resolverIndex++)
			{
