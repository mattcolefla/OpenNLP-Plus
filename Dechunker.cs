...
namespace OpenNLP.Tools.Chunker
{
    ...
    public abstract class Dechunker : IDechunker
    {
        ...
        {
            var operations = GetDechunkerOperations(chunks);
            ...
            for (var i = 0; i < chunks.Length; i++)
            {
                ...
                else if (operations[i + 1] == DechunkOperation.MERGE_TO_LEFT || operations[i + 1] == DechunkOperation.MERGE_BOTH)
                {
                    isAppendSpace = false;
                    isAppendSplitMarker = true;
                }
                // if this token is move right, no space
                else if (operations[i] == DechunkOperation.MERGE_TO_RIGHT || operations[i] == DechunkOperation.MERGE_BOTH)
                {
                    isAppendSpace = false;
                    isAppendSplitMarker = true;
                }
