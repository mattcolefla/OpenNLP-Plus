...
namespace OpenNLP.Tools.Coreference.Mention
{
	...
	public sealed class PennTreebankHeadFinder : IHeadFinder
	{
		...
		public IParse GetHead(IParse parse)
		{
			...
			{
				var parts = parse.SyntacticChildren;
				//shallow parse POS
				if (parts.Count > 2)
				{
				...
				if (parts.Count > 1)
				{
                    if (parts[0].IsNounPhrase)
					{
						...
						var tok = childTokens[childTokens.Count - 1];
						if (tok.SyntacticType == PartsOfSpeechStrings.PossessiveEnding)
						{
							return null;
						}
					}
				...
				if (parts.Count > 1)
				{
					for (var currentPart = 1; currentPart < parts.Count - 1; currentPart++)
					{
                        if (parts[currentPart].IsToken && parts[currentPart].SyntacticType == PartsOfSpeechStrings.CoordinatingConjunction)
						{
							return null;
						}
					}
		...
		{
			mSkipSet.Add(PartsOfSpeechStrings.PossessiveEnding);
			mSkipSet.Add(PartsOfSpeechStrings.Comma);
			mSkipSet.Add(PartsOfSpeechStrings.ColonSemiColon);
			mSkipSet.Add(PartsOfSpeechStrings.SentenceFinalPunctuation);
			mSkipSet.Add(PartsOfSpeechStrings.RightCloseDoubleQuote);
			mSkipSet.Add("-RRB-");
			mSkipSet.Add("-RCB-");
		}
	}
